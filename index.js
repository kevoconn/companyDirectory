...
function viewRoles() {
  db.query("SELECTrole.id, role.title, department.name AS department, role.salary FROM role JOIN department ON role.department_id = department.id", function (err, results) {
    console.table(results);
  });
}
...
function addRole() {
  db.query("SELECT * FROM department", function (err, results) {
    ...
    inquirer.prompt([
      {
        message: "What is title of the position?",
        name: "title",
      },
      {
